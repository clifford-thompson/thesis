%  file:   tree_draw_sicstus.pl    
%
%   copied from site:  www.nyu.edu/pages/linguistics/ling.html
%   and file:       nuy.edu/pages/linguistics/... file chris.txt
%
%   THIS VERSION FOR SICSTUS PROLOG
%   modified to work with SICStus Prolog version 3.8.5
%   by Charles Brown  2000 November 23
%
%   USE:    ?- drucke_baum(Tree).
%
% *****************************************************************
%    Originally from  the Prolog book below (now out of print)
% NAMES:   Dr. Christoph Lehner
% emailto: chris@cl.uni-hildesheim.de
% PHONES:  +5121 88 33 77 � +89 260 37 54
% WEB-ADR: http://www.uni-hildesheim.de/~chlehn
%
% Program is taken from Prolog text book  (now out of print)
% Christoph Lehner: "Prolog und Linguistik",
%    Oldenbourg, M�nchen 1992, 2. Auflage.
%    � Christoph Lehner
%
...
% DECLARE OPERATORS  \=/2  and  not/1  AND THEIR USES AS PREDICATES
%  The operator  \=  is found in newer releases of SICStus.
%  :- op(200,xfx,'\=').
%  \=(X,Y)  :-   \+(X = Y).

:- op(900,fy,not).   % same precedence as \+
not(X):- \+(X).

% UTILITIES  (standard definitions)
append([],L,L).
append([A|R],B,[A|L]) :- append(R,B,L).
...
dr_zw(k(X,Pos),L,S1,S4) :-
                       ...
                       letzter_knoten(L,Pos2),
                       tab(Pos2 - S3 - 1),
                       write('\\'),   % NOTE syntax to print a backslash
                       S4 is Pos2 .

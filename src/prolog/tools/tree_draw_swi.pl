%  file:  tree_draw_swi.pl
%
%   copied from site:  www.nyu.edu/pages/linguistics/ling.html
%   and file:       nuy.edu/pages/linguistics/... file chris.txt
%
%   THIS VERSION IS FOR SWI PROLOG
%   modified to work with SWI Prolog
%   by Paul Cooley November 2000
%
...
/* Blaetter */
print_branches([X,Y|Rest],S):-
                Y \= [_|_],
                !,
                print_branches([Y|Rest],S).

print_branches([X],S):-
                !.

/* nicht-verzweigende, z.B. lexikalische Kategorien */
print_branches([k(X,Pos),L|Rest],S1):-
                knoten_zahl(L,1),
                !,
                tab(Pos-S1),
                write(|),
                S2 is Pos + 1 ,
                print_branches(Rest,S2).

/* normale Kategorien */
print_branches([X,[K|R]|Rest],S1):-
                !,
                dr_ae(X,[K|R],S1,S2),
                print_branches(Rest,S2).
...
% pretty_print(Tree) :- drucke_baum(Tree).
